...
unauthenticated registry while it is still available.
  ...
  * Click `New Service Accout` to create your registry service account
...
What you can build your container image locally via podman with steps as shown below.
...
We are going to build the container image on OpenShift and deploy it as an application.
